...
// GitHub serves the calendar weekday-major, not in date order — always derive
// row/column from each day's date, never from array index.
...
export function buildLayout(days: Day[]): Layout {
  if (days.length === 0) {
    return { cells: [], columns: 0, months: [] };
  }

  const times = days.map((d) => toUtcMs(d.date));
  const earliest = Math.min(...times);
  // First column starts on the Sunday on or before the first day.
